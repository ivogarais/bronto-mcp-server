...
class DashboardsApiAgentSpec(BaseModel):
    ...
    tools: list[AgentToolSpec] = Field(
        default_factory=lambda: [
            AgentToolSpec(
                name=DashboardsApiToolName.LIST_DASHBOARDS_BY_LOG.value,
                handler=DashboardsApiToolName.LIST_DASHBOARDS_BY_LOG.value,
                description="List dashboards by log ID.",
            ),
        ]
    )
