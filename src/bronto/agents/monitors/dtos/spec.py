...
class MonitorsAgentSpec(BaseModel):
    ...
    tools: list[AgentToolSpec] = Field(default_factory=lambda: [
        AgentToolSpec(name=MonitorsToolName.LIST_MONITORS_BY_LOG.value, handler=MonitorsToolName.LIST_MONITORS_BY_LOG.value, description="List monitors by log ID."),
    ])
